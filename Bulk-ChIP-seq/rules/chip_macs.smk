...
rule chip_callpeak:
    ...
    shell:
        "macs3 callpeak --outdir {OUT_DIR}/Analysis --cutoff-analysis -n {params.name} {macs3_callpeak_option} -t {input.bam} -c {input.cbam}; "
...
rule chip_signaltracks:
    ...
    params:
        name = "{name}",
    shell:
        "macs3 bdgcmp -t {input.bdg_raw_t} -c {input.bdg_raw_c} -m ppois logFE -p 0.01 -o {output.bdg_pscore} {output.bdg_logfc};"
