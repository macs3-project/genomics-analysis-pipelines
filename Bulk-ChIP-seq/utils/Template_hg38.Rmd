...
write.table( over.gss.flanking.peaks.symbol, file="nearby_genes.tsv" )
```
...
if ( dim(ER)[1] > 0 ) {
    dotplot(ER, title="Human Phenotype Ontology")
}
```
...
if ( dim(ER)[1] > 0 ) {
    dotplot(ER, title="TF targets Signatures")
}
```
...
if ( dim(ER)[1] > 0 ) {
    dotplot(ER, title="Curated Reactome Pathways")
}
```
...
if ( dim(ER)[1] > 0 ) {
    dotplot(ER, title="Curated Wikipathways")
}
```
...
if ( dim(ER)[1] > 0 ) {
    dotplot(ER, title="Curated Biocarta")
}
```
...
if ( dim(ER)[1] > 0 ) {
    dotplot(ER, title="ALL Curated Canonical Pathways")
}
```
