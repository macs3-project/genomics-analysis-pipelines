...
n.p.hallmark <- length(p.hallmark@result$ID)
if ( n.p.hallmark > 0 ) {
...
```{r fig.height=6, fig.width=12}
n.p.curated <- length(p.curated@result$ID)
if ( n.p.curated > 0 ) {
  n <- min(n.p.curated, 5) 
...
```{r fig.height=6, fig.width=12}
n.p.canonicalpathways <- length(p.canonicalpathways@result$ID)
...
```{r fig.height=6, fig.width=12}
n.p.regulatory <- length(p.regulatory@result$ID)
if ( n.p.regulatory > 0 ) {
...
```{r fig.height=6, fig.width=12}
n.p.oncology <- length(p.oncology@result$ID)
if ( n.p.oncology > 0 ) {
