...
```{r compilealldata}
# all data will be saved into a file called `alldata.normalized.tsv`
rownames(gene.info) <- gene.info$id
count.r <- rownames(counts.data)
count.all <- cbind(gene.info[count.r,], counts.data)
if ( ! dir.exists("Results")) { dir.create("Results")}
write.table(count.all, file="Results/alldata.normalized.tsv",row.names=FALSE) 
```
...
if ( n.p.ontology > 0 ) {
  ...
  gseaplot2(p.ontology, geneSetID = 1:n, pvalue_table = T) + ggtitle(paste0("GSEA: Top ", as.character(n), " ontology"))
}
```
