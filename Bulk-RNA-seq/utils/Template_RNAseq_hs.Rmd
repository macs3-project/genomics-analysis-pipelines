...
gseanalysis <- function (genes, gset2gene, label="", qvalueCutoff=0.2, seed=12345, minGSSize=10, maxGSSize=200 ) {
    ...
    d$id <- gsub("\\.\\d+","",d$id)
    rownames(d) <- d$id
    mapping <- bitr(rownames(d), fromType="ENSEMBL", toType="ENTREZID", OrgDb="org.Hs.eg.db", drop=T) 
    ...
    en.gsea <- setReadable(en.gsea, org.Hs.eg.db, keyType = "ENTREZID")
        ...
        #p <- gseaplot2(en.gsea, geneSetID = 1:n)
        #p <- do.call("grid.arrange", c(p)) + ggtitle("GSEA plot")
...
hgmtfile <- paste0(MSigDBfoldername,"/h.all.v7.5.1.entrez.gmt")   # hallmark gene sets
h2gene <- read.gmt(hgmtfile)

cgmtfile <- paste0(MSigDBfoldername,"/c2.all.v7.5.1.entrez.gmt")  # Curated gene sets
c2gene <- read.gmt(cgmtfile)

tgmtfile <- paste0(MSigDBfoldername,"/c3.all.v7.5.1.entrez.gmt")  # Regulatory target gene sets
t2gene <- read.gmt(tgmtfile)

ongmtfile <- paste0(MSigDBfoldername,"/c5.all.v7.5.1.entrez.gmt")  # ontology gene sets
on2gene <- read.gmt(ongmtfile)

ogmtfile <- paste0(MSigDBfoldername,"/c6.all.v7.5.1.entrez.gmt")  # oncogenic signature gene sets
...
We will use the gene sets from MSigDB. Here are the sets we will check:
...
p.ontology   <- gseanalysis( deg.data, on2gene, paste0( contrast.name,".ontology" ), qvalueCutoff = 0.5 )
p.oncogenic   <- gseanalysis( deg.data, o2gene, paste0( contrast.name,".oncogenic" ), qvalueCutoff = 0.5 )
```
...
if ( n.p.hallmark > 0 ) {
  ...
  gseaplot2(p.hallmark, geneSetID = 1:n, pvalue_table = T)
}
```
...
if ( n.p.curated > 0 ) {
  n <- min(n.p.curated, 5) 
  gseaplot2(p.curated, geneSetID = 1:n, pvalue_table = T)
}
```
...
if ( n.p.regulatory > 0 ) {
  ...
  gseaplot2(p.regulatory, geneSetID = 1:n, pvalue_table = T)
}
```

## GSEA Ontology gene sets

```{r fig.height=6, fig.width=12}
n.p.ontology <- ifelse(!is.null(p.ontology), length(p.ontology@result$ID), 0 )

if ( n.p.ontology > 0 ) {
  n <- min(n.p.ontology, 5) 
  gseaplot2(p.ontology, geneSetID = 1:n, pvalue_table = T)
}
```

## GSEA Tumor Oncogenic gene sets

```{r fig.height=6, fig.width=12}
n.p.oncogenic <- ifelse(!is.null(p.oncogenic), length(p.oncogenic@result$ID), 0 )

if ( n.p.oncogenic > 0 ) {
  n <- min(n.p.oncogenic, 5) 
  gseaplot2(p.oncogenic, geneSetID = 1:n, pvalue_table = T)
}
```
...
id.all.entrez <- bitr(id.all.ensembl, fromType="ENSEMBL", toType=c("ENTREZID"), OrgDb="org.Hs.eg.db")$ENTREZ

id.up <- e.up$log2FoldChange
names(id.up) <- e.up$ensembl
id.up.ensembl <- names(id.up)
id.up.entrez <- bitr(id.up.ensembl, fromType="ENSEMBL", toType=c("ENTREZID"), OrgDb="org.Hs.eg.db")$ENTREZ

id.down <- e.down$log2FoldChange
names(id.down) <- e.down$ensembl
id.down.ensembl <- names(id.down)
id.down.entrez <- bitr(id.down.ensembl, fromType="ENSEMBL", toType=c("ENTREZID"), OrgDb="org.Hs.eg.db")$ENTREZ
```
...
```{r cp0}
go.all <- enrichGO(id.all.ensembl, c(id.all.ensembl, backM.data$ensembl),OrgDb = org.Hs.eg.db, keyType = "ENSEMBL", ont="BP", pAdjustMethod = "BH", pvalueCutoff = 0.05, qvalueCutoff = 0.2, readable=T)
...
```{r cp1}
go.up <- enrichGO(id.up.ensembl, c(id.up.ensembl, backM.data$ensembl),OrgDb = org.Hs.eg.db, keyType = "ENSEMBL", ont="BP", pAdjustMethod = "BH", pvalueCutoff = 0.05, qvalueCutoff = 0.2, readable=T)
...
```{r cp2}
go.down <- enrichGO(id.down.ensembl, c(id.down.ensembl, backM.data$ensembl),OrgDb = org.Hs.eg.db, keyType = "ENSEMBL", ont="BP", pAdjustMethod = "BH", pvalueCutoff = 0.05, qvalueCutoff = 0.2, readable=T)
...
```{r kegg0}
kegg.all <- enrichKEGG(id.all.entrez, c(id.all.entrez, backM.data$entrez), organism="hsa", pAdjustMethod = "BH", pvalueCutoff = 0.05, qvalueCutoff = 0.2, keyType="kegg")
...
```{r kegg0.draw, eval=FALSE, fig.width=16, include=FALSE}
v <- deg.data$log2FoldChange
names(v) <- deg.data$ensembl
for ( keggid in kegg.all$ID ) {
  keggpathview <- pathview(gene.data  = v, pathway.id = keggid, gene.idtype="ENSEMBL", species = "hsa" , limit=list(gene=2, cpd=1) )
...
keggpvfiles <- list.files(path = './', pattern = "^hsa([[:digit:]]+).pathview.png$", full.names = TRUE)
